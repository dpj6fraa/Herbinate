...
function EmailExistsError({ setError }: { setError: (msg: string) => void }) {
  const searchParams = useSearchParams();

  useEffect(() => {
    if (searchParams.get("error") === "email_exists") {
      setError("อีเมลนี้มีบัญชีอยู่แล้ว กรุณาเข้าสู่ระบบด้วยอีเมลและรหัสผ่าน");
    }
...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-svh bg-white flex flex-col">
      {/* ✅ Suspense บังคับโดย Next.js สำหรับ useSearchParams */}
      <Suspense fallback={null}>
        <EmailExistsError setError={setError} />
      </Suspense>
          ...
          <div className="w-full max-w-md">
            ...
            <div className="flex justify-between items-center mb-6 text-sm">
              <label className="flex gap-2 items-center text-black cursor-pointer group">
                <input
                  ...
                  disabled={loading}
                  className="
                    cursor-pointer appearance-none w-4 h-4 rounded
            ...
            <div className="flex justify-center gap-4 mb-4">
              <button
                ...
                className="w-12 h-12 border border-gray-300 hover:bg-gray-50 rounded-full flex items-center justify-center cursor-pointer transition-colors"
              >
                <Image src="/google.webp" alt="Google" width={24} height={24} />
              </button>
            </div>
