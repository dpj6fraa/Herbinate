...
export default function ComparePage() {
  ...
  const handleCompare = () => {
    if (selectedHerbs.length >= 2) {
      // Use query string to pass IDs to the result page
      const query = selectedHerbs.join(",");
      router.push(`/Compare/result?herbs=${query}`);
    }
  };
        ...
        <div className="flex gap-2 mb-4 overflow-x-auto pb-2 scrollbar-hide">
          {filters.map(filter => (
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-[11px] sm:text-xs whitespace-nowrap border transition-colors flex items-center gap-1.5 shrink-0 ${
                activeFilter === filter
            ...
            {filteredHerbs.map(herb => {
              ...
              return (
                <div
                  ...
                  className={`flex flex-col bg-white rounded-xl overflow-hidden transition-all duration-200 border relative ${
                    isSelected 
      ...
      {selectedHerbs.length > 0 && (
        <div className="fixed bottom-8 right-6 z-40 transform transition-all duration-300 animate-in slide-in-from-bottom-5">
          <button
            ...
            className={`shadow-lg rounded-xl px-5 py-3 font-bold text-sm sm:text-base flex items-center gap-2 transition-all ${
